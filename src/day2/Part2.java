		...
		*/
		...
		for(int row=0; row < spreadsheet.length; row++) {
			for(int column=0; column < spreadsheet[0].length; column++) {
				found = false;
				currentNumber = spreadsheet[row][column];
				for(int columnSecond=0; columnSecond < spreadsheet[0].length; columnSecond++) {
					secondNumber = spreadsheet[row][columnSecond];
					if(column != columnSecond && found == false) {
						if(currentNumber % secondNumber == 0) {
							checksum += currentNumber / secondNumber;
							found = true;
						}
					}
				}
